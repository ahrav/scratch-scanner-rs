## Streaming Vectorscan parity checklist

Goal: converge our streaming decode path in **priority order**. This list is the checklist we’ll implement _one item at a time_. Each item includes scope, invariants, and acceptance checks.

### P0 — correctness + safety (do first)

- [x] **Window loss recovery when the ring can’t serve `[lo, hi)`**
...
- [x] **Per-rule stream hit caps + force-full fallback**
...
- [x] **Unbounded expression sentinel handling in stream mode**
...
- [x] **Nested span capture fallback (don’t drop spans)**
...
- [x] **UTF‑16 anchor scanning in decoded streams (lazy stream DB)**
...
- [x] **Decoded gate parity (optional `VsGateDb`)**
...
- [x] **Stream pending window cap telemetry**
