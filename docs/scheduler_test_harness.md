...
In stress runs, the harness writes a repro artifact to `tests/scheduler_failures/seed_<seed>.json` before failing.

### Debugging playbook

1. **Locate the artifact** in `tests/scheduler_failures/` and inspect `case`, `driver_choices`, and `failure` (kind, step, message).
