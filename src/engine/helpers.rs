...
///
/// # Returns
/// - `EntropyMetrics { shannon_bpb: 0.0, max_bin_count: 0 }` for empty input.
#[inline]
pub(super) fn compute_entropy_metrics(
    bytes: &[u8],
...
pub(super) fn entropy_gate_passes(
    ...
    if let Some(me_min) = spec.min_entropy_bits_per_byte {
        ...
        let me =
            log2_lookup(log2_table, n) - log2_lookup(log2_table, metrics.max_bin_count as usize);
        if me < me_min {
            return false;
        }
    }
