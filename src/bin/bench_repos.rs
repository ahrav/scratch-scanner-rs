...
use scanner_rs::{
    demo_engine_with_anchor_mode, AnchorMode, FileId, ScannerConfig, ScannerRuntime,
    BUFFER_LEN_MAX,
};
use std::env;
use std::ffi::OsStr;
...
fn main() -> io::Result<()> {
    ...
    let exe = args.get(0).map(|s| s.as_str()).unwrap_or("bench_repos");
    ...
    for repo in repos.iter() {
        ...
        total_errors = total_errors.saturating_add(res.errors);
    }
